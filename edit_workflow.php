...
<style>
  ...
  .cw-header{
    ...
    margin-bottom:20px;backdrop-filter:blur(12px);transition:box-shadow .25s;
  }
  ...
  .cw-panel{background:var(--cw-surface);border:1px solid var(--cw-border);border-radius:16px;overflow:hidden;}
  .cw-panel-full{grid-column:1/-1;}
  .cw-panel-head{padding:14px 18px;border-bottom:1px solid var(--cw-border);display:flex;align-items:center;gap:10px;}
  ...
  .cw-input:focus,.cw-select:focus,.cw-textarea:focus{outline:none;border-color:var(--cw-primary);box-shadow:0 0 0 4px var(--cw-primary-light);}
  ...
  .cw-cond-box{border:1px solid var(--cw-border);border-radius:14px;padding:16px;background:var(--cw-primary-light);margin-top:14px;}
  .cw-cond-icon{
    width:44px;height:44px;border-radius:12px;background:var(--cw-primary-light);color:var(--cw-primary);
  ...
  .cw-err{
    ...
    color:#b91c1c;border-radius:12px;padding:12px 14px;font-size:13px;margin-top:16px;line-height:1.5;
  }
...
<div class="main-content app-content cw">
  <div class="cw-wrap">
    ...
    <input type="hidden" id="workflow_id" value="<?php echo htmlspecialchars($id); ?>">
    ...
    <div class="cw-header" id="cwHeader">
      <div class="cw-header-left">
        ...
        <div>
          <h1>Edit Workflow</h1>
          <p>Modify conditions and email template</p>
        </div>
      </div>
      ...
      <div class="cw-panel">
        ...
        <div class="cw-panel-body">
          <div class="cw-field">
            ...
            <input id="workflow_name" class="cw-input" />
          </div>
          ...
          <div class="cw-field">
            ...
            <textarea id="description" class="cw-textarea"></textarea>
          </div>
        </div>
      </div>
...
async function loadEmailTemplates(selectedId){
...
function onConditionTypeChange(statusVal, eventField){
  ...
  if(type === "status_equals"){
    ...
    if(statusVal) document.getElementById("status_value").value = statusVal;
    return;
  }

  if(type === "event_date"){
    let dir = 'before', days = 1;
    if(eventField && eventField.includes('|')){
    ...
    let daysHtml = '';
    for(let i=1;i<=30;i++){
      daysHtml += '<button type="button" class="cw-day-btn'+(i===days?' active':'')+'" data-day="'+i+'" onclick="selectDay(this,'+i+')">'+i+'</button>';
    }
    host.innerHTML = `
      <div class="cw-cond-box">
        <div class="cw-cond-icon">
        ...
        <div class="cw-field">
          ...
          <div class="cw-dir-group">
            <button type="button" class="cw-dir-btn${dir==='before'?' active':''}" data-dir="before" onclick="selectDirection(this,'before')">
            ...
            <button type="button" class="cw-dir-btn${dir==='after'?' active':''}" data-dir="after" onclick="selectDirection(this,'after')">
              After Event
        ...
        <div class="cw-hint">The email will be sent <strong><span id="dirLabel">${dir}</span></strong> the event date by <strong><span id="daysLabel">${days}</span></strong> day(s).</div>
      </div>`;
    return;
  }
...
async function loadWorkflow(){
  setErr("");
  const id = document.getElementById("workflow_id").value;
  if(!id) return setErr("Missing workflow id in URL.");

  const res = await postForm(SUITE_ENTRYPOINT, {action:"fetch_single_workflow", id});
  const text = await res.text();
  let json;
  try{ json = JSON.parse(text); }catch(e){ return setErr("Invalid server response."); }
  if(!json.success) return setErr(json.message || "Failed to load workflow.");

  const w = json.workflow || {};
  document.getElementById("workflow_name").value = w.workflow_name || "";
  document.getElementById("module_name").value = w.module_name || "Leads";
  document.getElementById("status").value = w.status || "Active";
  document.getElementById("description").value = w.description || "";

  document.getElementById("condition_type").value = w.condition_type || "on_create";
  onConditionTypeChange(w.status_value || "", w.event_date_field || "");

  await loadEmailTemplates(w.email_template_id || "");
}
...
async function saveWorkflow(){
  ...
  try{
    ...
    if(condType === "status_equals" && !payload.status_value){
      ...
      return setErr("Please select a status value.");
    }

    const res = await postForm(SUITE_ENTRYPOINT, {action:"update_workflow", ...payload});
    const text = await res.text();
    let json;
    try{ json = JSON.parse(text); }catch(e){ return setErr("Invalid server response."); }
    ...
    if(typeof Swal !== 'undefined'){
      await Swal.fire({icon:'success',title:'Updated',text:'Workflow saved successfully.',timer:1800,showConfirmButton:false});
    }
    await loadWorkflow();
  }catch(e){
