...
foreach ($leads as $lead) {
  ...
  if ($dateObj) {
    ...
    if (isset($trendData[$ym])) $trendData[$ym] += $revenue;
  }
...
<style>
  ...
  /* Funnel Cards */
  .ra-funnel-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; }
  .ra-funnel-card { background: var(--ra-surface); border: 1px solid var(--ra-border); border-radius: 14px; padding: 20px 16px; text-align: center; position: relative; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; }
  .ra-funnel-card:hover { transform: translateY(-3px); box-shadow: 0 8px 30px rgba(15,23,42,0.08); }
  .dark .ra-funnel-card:hover { box-shadow: 0 8px 30px rgba(0,0,0,0.3); }
  .ra-funnel-card-glow { position: absolute; top: -24px; right: -24px; width: 80px; height: 80px; border-radius: 50%; opacity: 0.10; pointer-events: none; }
  .ra-funnel-card-icon { width: 44px; height: 44px; border-radius: 12px; display: inline-flex; align-items: center; justify-content: center; font-size: 20px; margin: 0 auto 12px; }
  .ra-funnel-card-count { font-size: 28px; font-weight: 900; color: var(--ra-text); line-height: 1; letter-spacing: -0.02em; }
  .ra-funnel-card-label { font-size: 12px; font-weight: 700; color: var(--ra-muted); margin-top: 6px; text-transform: uppercase; letter-spacing: .04em; }
  .ra-funnel-card-pct { display: inline-flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 20px; margin-top: 10px; }
  .ra-funnel-card-drop { font-size: 10px; color: #ef4444; font-weight: 700; margin-top: 4px; }
  .ra-funnel-card-bar { height: 4px; border-radius: 2px; margin-top: 14px; opacity: 0.30; transition: width 0.8s ease; }
  .ra-funnel-arrow { display: flex; align-items: center; justify-content: center; align-self: center; color: var(--ra-muted); font-size: 18px; opacity: 0.35; }
  ...
  @media (max-width: 768px) {
    ...
    .ra-funnel-grid { grid-template-columns: repeat(2, 1fr); }
    .ra-funnel-arrow { display: none; }
  }
      ...
      <div class="ra-card-body">
        ...
        <div class="ra-funnel-grid">
          <?php foreach ($funnelTiers as $idx => $tier):
            $total = $funnelData['total'] ?: 1;
            $pct = round(($tier['count'] / $total) * 100, 1);
            $barWidth = max($pct, 6);
            $dropPct = ($idx > 0 && $prevCount > 0) ? round((1 - $tier['count'] / $prevCount) * 100, 1) : 0;
          ?>
            <div class="ra-funnel-card">
              <div class="ra-funnel-card-glow" style="background:<?php echo $tier['color']; ?>;"></div>
              <div class="ra-funnel-card-icon" style="background:<?php echo $tier['color']; ?>15;color:<?php echo $tier['color']; ?>;">
                <i class="<?php echo $tier['icon']; ?>"></i>
              </div>
              <div class="ra-funnel-card-count"><?php echo number_format($tier['count']); ?></div>
              <div class="ra-funnel-card-label"><?php echo $tier['label']; ?></div>
              <div class="ra-funnel-card-pct" style="background:<?php echo $tier['color']; ?>12;color:<?php echo $tier['color']; ?>;"><?php echo $pct; ?>%</div>
              <?php if ($idx > 0 && $dropPct > 0): ?>
                <div class="ra-funnel-card-drop"><i class="ri-arrow-down-s-line" style="font-size:11px;vertical-align:middle;"></i> -<?php echo $dropPct; ?>%</div>
              <?php endif; ?>
              <div class="ra-funnel-card-bar" style="width:<?php echo $barWidth; ?>%;background:<?php echo $tier['color']; ?>;max-width:100%;"></div>
            </div>
    ...
    <div class="ra-card mb-6">
      <div class="ra-card-header">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-bold text-sm" style="color:var(--ra-text)">Revenue Trend (24 Months)</div>
            <div style="font-size:11px;color:var(--ra-muted);">Monthly gross revenue based on lead creation date</div>
          </div>
        </div>
      </div>
...
function setPeriod(period, btn) {
  ...
  document.querySelectorAll('.ra-filter-btn').forEach(function(b) { b.classList.remove('active'); });
...
(function() {
  ...
  var shortLabels = trendLabels.map(function(l) {
    var parts = l.split('-');
    var months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    return months[parseInt(parts[1],10)-1] + ' ' + parts[0].slice(2);
  });

  var ctx = document.getElementById('ra-revenue-chart').getContext('2d');
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: shortLabels,
    ...
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { color: textColor, font: { size: 10, weight: 600 }, maxRotation: 45 }
        },
        y: {
